/*
  Warnings:

  - You are about to drop the column `endDateTune` on the `Reservation` table. All the data in the column will be lost.
  - Added the required column `endDateTime` to the `Reservation` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE "Reservation" DROP COLUMN "endDateTune",
ADD COLUMN     "endDateTime" TIMESTAMP(3) NOT NULL;
